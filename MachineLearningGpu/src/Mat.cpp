...
void Mat::SetData(const float* data)
{
	bo.SetSubData((void*)data, Size() * sizeof(float), sizeof(size));
}
...
Mat::Mat(unsigned rows, unsigned cols, const std::vector<float>& data)
{
	size.x = rows;
	size.y = cols;
	InitBlank();
...
std::vector<float> Mat::GetRow(unsigned int row) const
{
	std::vector<float> data(size.y);
	unsigned int rowSize = size.y * sizeof(float);
	bo.GetSubData(&data[0], row*rowSize + sizeof(size), rowSize);
	return data;
}
